        ...
        $(window).load(function () {
            ...
            $('#slider-' + id).flexslider({
                ...
                before: function (slider) {
                    var handler = $('.slider li').eq(slider.animatingTo).find('img');
            ...
            function imgLoader(index) {
                var handler = $('.slider .flex-active-slide img');
                var img = new Image();
                img.src = $(handler).attr('data-original');
                img.onload = function () {
                    var handler = $('.slider li').eq(index).find('img');
                    if (handler.attr('src') != handler.attr('data-original')) {
                        $('.slider').spin(false);
                    }
                    $(handler).attr('src', img.src);
                    var aspectRatioContainer = $('.slider').width()/$('.slider').height();
                    var aspectRatioImg = img.width/img.height;
                    if (aspectRatioContainer>=aspectRatioImg){
                           $(handler).attr('width',$('.slider').width());
                         $(handler).css('top',($('.slider').height()-$('.slider .flex-active-slide').height())/2);

                    }
                    else{
                          $(handler).attr('height',$('.slider').height());
                         $(handler).css('left',($('.slider .flex-active-slide').width()-$('.slider').width())/2);
        ...
        <div id="slider-<?php echo($gallery->ID); ?>" class="flexslider slider">
            <ul class="slides">
                <?php foreach ($images as $image) : ?>
                    ...
                    <li>
                        <a href="<?php echo $image->imageURL ?>" title="<?php echo $gallery->description ?>">
