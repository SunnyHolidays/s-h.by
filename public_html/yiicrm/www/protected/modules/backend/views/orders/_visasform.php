    ...
    <div class="controls">
        ...
        echo $form->dropDownList(
            ...
            array(
                ...
                'id' => 'participantList',
                'options' => array(key($data) => array('selected' => true)),
            )
        ); ?>
...
<script>
    function customRenderErrorMessage(form, data, hasError) {
        if (renderErrorMessage(form, data, hasError)) {
            $.ajax({
                type: 'post',
                url: "<?php echo !$model->isNewRecord ? Yii::app()->createUrl('/backend/orders/visasupdate', array('id' => $model->id)) :
                                Yii::app()->createUrl('/backend/orders/visascreate')?>",
