...
<div id="gallery">
  <ul class="portfolio">
    ...
    ?>
      ...
      <?php }else{ ?>
        <span class="image-border"><a class="image-wrap" href="<?php the_permalink() ?>" title="<?php _e('Permanent Link to', 'theme1404');?> <?php the_title_attribute(); ?>" ><?php the_post_thumbnail( 'portfolio-post-thumbnail' ); ?></a></span>
        <?php } ?>
        <div class="folio-desc">
          <h6><a href="<?php the_permalink(); ?>"><?php $title = the_title('','',FALSE); echo substr($title, 0, 40); ?></a></h6>
          <?php $excerpt = get_the_excerpt(); echo my_string_limit_words($excerpt,6);?>
        </div>
      </li>
