...
class MY_CommentWidget extends WP_Widget_Recent_Comments {
	...
	function MY_CommentWidget() {
		$widget_ops = array('classname' => 'widget_my_recent_comments', 'description' => __('My - Recent Comments','theme1404') );
		$this->WP_Widget('my-recent-comments', __('My - Recent Comments','theme1404'), $widget_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __('My Recent Comments','theme1404') : $instance['title']);
...
?>
		<?php echo $before_widget; ?>
			...
			<br />
			<?php 
			echo __('by', 'theme1404'). '&nbsp;' . sprintf(_x('%1$s on %2$s', 'widgets'), get_comment_author_link(), '<a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '">' . get_the_title($comment->comment_post_ID) . '</a>'); ?>
